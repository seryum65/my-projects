...
def calculate():
    if request.method == "POST":
        ...
        return render_template("result.html", result1 = num1, result2 = num2, lcm = lcm(int(num1),int(num2)), developer_name = 'Yusuf')
    else:
        return render_template("result.html", developer_name = 'Yusuf')


# Add a statement to run the Flask application which can be debugged.

if __name__== "__main__":
    #app.run(debug=True) # Lokalde çalışırken bunu aktif et
    app.run(host='0.0.0.0', port=80) # ec-2 da port 80 de çalışma kodu